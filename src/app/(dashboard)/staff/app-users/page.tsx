...
import {
  ...
  AlertCircle, MoreVertical, ToggleLeft, ToggleRight, Copy, KeyRound,
  Zap, History, UserPlus, RefreshCw, Activity,
} from "lucide-react";
...
const ROLES: {
...
}[] = [
  {
    value: "super_admin",
    label: "Super Admin",
    description: "Full access + activity log for all users",
  ...
  },
  {
    value: "cashier",
    label: "Cashier",
    description: "Can create orders and view sells",
...
function PermBadge({ level }: { level: PermLevel }) {
  if (level === "full")
    return <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-green-100 text-green-600"><Check size={12} /></span>;
  if (level === "view")
    return <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-blue-50 text-blue-400"><Eye size={11} /></span>;
...
const EMPTY_FORM = { email: "", name: "", password: "", role: "cashier" as AppRole, notes: "" };
...
// Run this if the tables already exist (adds super_admin role + creates activity_logs)
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-amber-900">
                {isRLS ? "Permission error — RLS policy missing" : "One-time setup required"}
              </h3>
              <p className="text-sm text-amber-700 mt-1">
                {isRLS
                  ? "The table exists but Row Level Security is blocking writes. Run the SQL below in your Supabase SQL Editor to fix it."
          ...
          {showSQL && (
            <div className="relative">
              <pre className="bg-gray-900 text-green-400 text-xs p-4 rounded-xl overflow-x-auto leading-relaxed font-mono">
                {isRLS ? RLS_FIX_SQL : SETUP_SQL}
              </pre>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(isRLS ? RLS_FIX_SQL : SETUP_SQL);
          ...
          <div className="flex items-center gap-3 pt-1">
            <p className="text-xs text-amber-600 flex-1">
              Steps: Open Supabase Dashboard &rarr; SQL Editor &rarr; paste &amp; run &rarr; refresh.
            </p>
            <button
              onClick={fetchUsers}
              className="h-8 px-4 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold transition-colors shrink-0"
            >
              Retry
            </button>
          </div>
          ...
          <div className="bg-white border border-border rounded-2xl p-6 space-y-3">
            ...
            {showRLSFix && (
              <div className="relative">
                ...
                <button
                  onClick={() => { navigator.clipboard.writeText(SETUP_SQL); toast.success("SQL copied!"); }}
                  className="absolute top-2 right-2 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-white text-xs font-medium transition-colors"
                >
                  <Copy size={11} /> Copy
                </button>
...
export default function AppUsersPage() {
  ...
  // Track which emails have a real Supabase Auth account
  const [authEmails, setAuthEmails] = useState<Set<string>>(new Set());
  // Current logged-in user (for activity logging + showing as Super Admin)
  ...
  const fetchUsers = async () => {
    ...
    if (error) {
      console.error("[app_user_roles fetch]", error);
      if (isTableMissingError(error)) {
        setTableReady("missing");
      } else if (isPermissionError(error)) {
        setTableReady("rls");
      } else {
        setTableReady("missing"); // treat unknown errors as setup needed
      }
      setLoading(false);
      return;
    }
    ...
    if (list.length > 0) {
      ...
      fetch(`/api/app-users?emails=${encodeURIComponent(emails)}`)
        .then((r) => r.json())
        .then((json) => {
          if (json.authEmails) setAuthEmails(new Set(json.authEmails as string[]));
        })
        .catch(() => {});
    }
  };

  useEffect(() => { fetchUsers(); }, [rid]);
  ...
  useEffect(() => {
    if (showActivityLog && rid) fetchActivityLogs();
  }, [showActivityLog, rid]);
  ...
  const logActivity = async (
    ...
    } catch { /* silently ignore — don't block the main action */ }
  };
  ...
  const openEdit = (u: AppUser) => {
    setEditing(u);
    setForm({ email: u.email, name: u.name, role: u.role, notes: u.notes ?? "", password: "" });
    setModalOpen(true);
  };

  // Edit the account owner (current logged-in user) — no app_user_roles record
  const openEditOwner = () => {
    if (!currentUser) return;
    setEditing({ id: "__owner__", email: currentUser.email, name: currentUser.name, role: "super_admin", is_active: true, notes: null, restaurant_id: rid ?? "", created_at: "" });
    setForm({ email: currentUser.email, name: currentUser.name, role: "super_admin", notes: "", password: "" });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    // ── Owner editing their own account ──────────────────────────────────────
    if (editing?.id === "__owner__") {
      const supabase = createClient();
      const updates: { data?: { full_name: string }; password?: string } = {};
      updates.data = { full_name: form.name.trim() };
    ...
    if (editing) {
      // PATCH — update role/name/notes + optional password reset
      ...
      if (!res.ok) { toast.error(json.error ?? "Failed to update user"); setSaving(false); return; }
      // Log what changed
      if (editing.role !== form.role) {
        await logActivity("user.role_changed", form.name.trim(), { old_role: editing.role, new_role: form.role });
      } else if (form.password.trim()) {
        await logActivity("user.password_reset", form.name.trim());
      } else {
        await logActivity("user.updated", form.name.trim());
      }
      toast.success(form.password.trim() ? "User updated & login credentials set" : "User updated");
    } else {
      // POST — create auth user + role record
      const res = await fetch("/api/app-users", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const toggleActive = async (u: AppUser) => {
    ...
    await logActivity(u.is_active ? "user.deactivated" : "user.activated", u.name);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const { error } = await createClient().from("app_user_roles").delete().eq("id", confirmDelete.id);
    if (error) { toast.error("Failed to delete"); setDeleting(false); return; }
    await logActivity("user.deleted", confirmDelete.name, { email: confirmDelete.email, role: confirmDelete.role });
  ...
  if (tableReady === "missing" || tableReady === "rls") {
    return (
      <TableSetupView
        ...
        showRLSFix={showRLSFix}
        setShowRLSFix={setShowRLSFix}
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5">
        ...
        <div className="bg-white border border-border rounded-xl p-3 flex items-center justify-between gap-3 flex-wrap">
          {/* Left: role filter + add */}
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
              <button
                onClick={() => setRoleFilter("")}
                className={`h-7 px-3 rounded-md text-xs font-medium transition-all ${roleFilter === "" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
              >
                All
              </button>
              {ROLES.map((r) => (
            ...
            <button
              onClick={openAdd}
              className="h-8 px-3 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium flex items-center gap-1.5 transition-colors shrink-0"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="h-8 pl-8 pr-3 w-56 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-gray-50"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
          {ROLES.map((role) => {
            ...
            const count = users.filter((u) => u.role === role.value).length
              + (role.value === "super_admin" && currentUser && !users.some((u) => u.email.toLowerCase() === currentUser.email.toLowerCase()) ? 1 : 0);
            return (
                      ...
                      <tr key={row.module} className="hover:bg-gray-50/40">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2 text-sm text-gray-700 font-medium">
                            <Icon size={13} className="text-gray-400" /> {row.module}
                          </div>
                        </td>
                        {ROLES.map((r) => (
                          <td key={r.value} className="px-4 py-3 text-center">
                            <PermBadge level={row[r.value]} />
                          </td>
                        ))}
                      </tr>
                    );
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[640px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {/* Current logged-in user — shown as Super Admin (You) */}
                  {currentUser && !users.some((u) => u.email.toLowerCase() === currentUser.email.toLowerCase()) && (
                    <tr className="bg-amber-50/40 hover:bg-amber-50/60 transition-colors">
                      ...
                      <td className="px-4 py-3.5"><RoleBadge role="super_admin" /></td>
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {!authEmails.has(u.email.toLowerCase()) && (
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700" title="No login account — edit this user and set a password to enable login">
        ...
        {/* Migration banner — only shown if user explicitly opens it or activity_logs errors */}
        {showMigrateSQL && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="relative mt-2">
                ...
                <button
                  onClick={() => { navigator.clipboard.writeText(MIGRATE_SQL); toast.success("SQL copied!"); }}
            ...
            <button onClick={() => setShowMigrateSQL(false)} className="text-amber-400 hover:text-amber-600 shrink-0">
              <X size={14} />
            </button>
          </div>
        )}

        {/* ── Activity Log (Super Admin view) ── */}
        <div className="bg-white border border-border rounded-xl overflow-hidden">
          <button
          ...
          >
            ...
            <div className="flex items-center gap-2">
              {showActivityLog && (
                <button
                  onClick={(e) => { e.stopPropagation(); fetchActivityLogs(); }}
                      ...
                      <div key={log.id} className="flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-1.5 flex-wrap">
                            ...
                            {log.target_label && (
                              <span className="text-xs text-gray-600 font-semibold">&ldquo;{log.target_label}&rdquo;</span>
                            )}
                          </div>
                          {log.details && (
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                              {log.details.old_role && log.details.new_role ? (
                                ...
                                <span className="text-[10px] text-gray-400">Role: {log.details.role}</span>
                              ) : null}
                              {log.details.email && (
                                <span className="text-[10px] text-gray-400">{log.details.email}</span>
                              )}
                            </div>
                          )}
                        </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                {/* Show warning if editing a user who has no auth account yet */}
                {editing && !authEmails.has(editing.email.toLowerCase()) && (
                  <div className="flex items-start gap-2 p-2.5 rounded-lg bg-amber-50 border border-amber-200 mb-2">
                    ...
                    <p className="text-xs text-amber-700 leading-snug">
                      <span className="font-semibold">Login not set up.</span> Set a password below to activate this user&apos;s login credentials.
                    </p>
                  </div>
                )}
                <input
                ...
                />
                {!editing && (
                  <p className="text-xs text-gray-400 mt-1">This password lets the user sign in to the app.</p>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {(editing?.role === "super_admin" || editing?.id === "__owner__") && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">
                      <Zap size={9} /> Role locked — cannot downgrade Super Admin
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {ROLES.map((role) => {
                    ...
                    const isRoleLocked = editing?.role === "super_admin" || editing?.id === "__owner__";
                    return (
                      <button
                        key={role.value}
                        type="button"
                        disabled={isRoleLocked}
                        onClick={() => !isRoleLocked && setForm((f) => ({ ...f, role: role.value }))}
                        className={`flex items-start gap-2.5 p-3 rounded-xl border-2 text-left transition-all ${
                          isRoleLocked
                            ? active ? `${role.border} ${role.bg} opacity-90` : "border-gray-100 bg-gray-50 opacity-40 cursor-not-allowed"
                            : active
                              ? `${role.border} ${role.bg}`
              ...
              {/* Notes — not shown for account owner */}
              {editing?.id !== "__owner__" && <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Weekend shift cashier"
                  rows={2}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/60 rounded-b-2xl">
              <button onClick={closeModal} className="h-9 px-4 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button
                onClick={handleSave}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Remove <strong>{confirmDelete.name}</strong> from this restaurant's access?
                </p>
              </div>
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setConfirmDelete(null)} className="h-9 px-4 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button
                onClick={handleDelete}
                disabled={deleting}
                className="h-9 px-4 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-semibold disabled:opacity-50 transition-colors"
              >
